...
def augment_data(img_train: np.uint8, lbl_train: np.uint8, img_test: np.uint8, lbl_test: np.uint8) -> tuple[tuple[np.uint8, np.uint8], tuple[np.uint8, np.uint8]]:
    ...
    img_train_augmented = np.concatenate(img_train, img_train_shifted, img_train_noisy, img_train_blurred, img_train_synthetic)
    lbl_train_augmented = np.concatenate(lbl_train, lbl_train_shifted, lbl_train_noisy, lbl_train_blurred, lbl_train_synthetic)
    img_test_augmented = np.concatenate(img_test, img_test_shifted, img_test_noisy, img_test_blurred, img_test_synthetic)
    lbl_test_augmented = np.concatenate(lbl_test, lbl_test_shifted, lbl_test_noisy, lbl_test_blurred, lbl_test_synthetic)
